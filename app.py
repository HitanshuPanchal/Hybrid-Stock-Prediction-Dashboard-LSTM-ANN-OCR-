...
if train_lstm:
    ...
    try:
        ...
        st.error("No LSTM model found. Train the model first.")
        st.stop()
...
try:
    ...
    st.error("Sentiment model files missing! Run sentiment_ann.py")
    st.stop()
...
future = []
last_seq = scaled[-seq_len:].copy()     # use last window of scaled data
...
for _ in range(7):
    ...
    # Convert scaled prediction back to real close price
    ...
    # update only CLOSE price in scaled form
    new_row[3] = pred_scaled[0][0]

    # OPTIONAL: keep technical indicators consistent
    # (last known values are used)
    # new_row[5:] = last_seq[-1][5:]

    # Slide the 60-day window
